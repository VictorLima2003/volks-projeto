...
export function GerenciadorHooks({
  ...
  async function salvar() {
    ...
    const res = await fetch("/api/hooks", {
      ...
      body: JSON.stringify({ campanhaId, hooks }),
    });
...
function EdicaoHook({
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <ExecutarTeste campanhaId={campanhaId} hook={hook} />
      </Card>
...
function ExecutarTeste({ campanhaId, hook }: { campanhaId: string; hook: Hook }) {
  ...
  async function testar() {
    ...
    const res = await fetch("/api/executar-hook", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ campanhaId, hookAvulso: hook, argumentos: args }),
    });

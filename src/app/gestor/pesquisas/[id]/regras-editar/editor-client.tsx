...
import { GrupoDeFatos } from "@/lib/engine";
import { EditorCondicao } from "@/components/EditorCondicao";
import { acharDesfecho, Condicao, Desfecho, Operador, Regra } from "@/lib/types";
import { useAviso } from "@/components/Avisos";
import { useRouter } from "next/navigation";
import { useState } from "react";

/** O selo segue o tom que o autor deu ao desfecho, não o nome dele. */
function tomDoDesfecho(tom: Desfecho["tom"]): "go" | "stop" | "warn" | "neutral" {
  return tom === "go" ? "go" : tom === "stop" ? "stop" : tom === "espera" ? "warn" : "neutral";
}
...
export function EditorRegras({
  ...
  grupos: GrupoDeFatos[];
  /** Os fins possíveis desta pesquisa — o que o seletor de resultado oferece. */
  desfechos: Desfecho[];
}) {
  ...
  async function publicar() {
    ...
    const res = await fetch("/api/rulesets", {
      ...
      body: JSON.stringify({ pesquisaId, regras, descricao }),
    });
  ...
  return (
    <div className="grid lg:grid-cols-4 gap-6 items-start">
          ...
          return (
            <Card key={r.id} className="p-0 overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-3 bg-ink-100 border-b hairline">
                ...
                <Badge tone={tomDoDesfecho(acharDesfecho(desfechos, r.resultado).tom)}>
                  {acharDesfecho(desfechos, r.resultado).rotulo}
                </Badge>
              ...
              <div className="p-6 space-y-5">
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      value={r.resultado}
                      onChange={(e) => patch(r.id, "resultado", e.target.value)}
                    >
                      {desfechos.map((d) => (
      ...
      <aside className="space-y-5 lg:sticky lg:top-28">
        <Card>
          <h3 className="text-sm font-semibold">Enviar para revisão</h3>
          <div className="mt-4">
            ...
            <p className="text-sm text-ink-700 mt-3">
              {regras.length} regra(s). A v{versaoAtual} continua valendo até alguém publicar esta.
            </p>
          </div>
          ...
          <div className="mt-5">
            ...
            <Textarea
              ...
              placeholder="Ex.: reduzido para 400 corridas na virada do semestre"
            />
            <p className="text-sm text-ink-600 mt-2">
              Quem for revisar lê este texto junto com o diff.
            </p>
          </div>
          ...
          <p className="text-xs text-ink-600 mt-3">
            Você não revisa a própria versão.
          </p>
        </Card>

...
export function VersoesPanel({
  ...
  async function reativar(ruleSetId: string) {
    ...
    const res = await fetch("/api/rulesets", {
      ...
      body: JSON.stringify({ pesquisaId, ruleSetId, acao: "reativar" }),
    });
        ...
        {versoes.map((v) => (
          ...
          >
            ...
            <dl className="mt-3 space-y-1 text-xs text-ink-600">
              ...
              {v.publicadoPor && (
                <div>
                  Publicada por <strong className="text-signal-go">{v.publicadoPor}</strong>{" "}
                  <span className="mono">{v.publicadoEm?.slice(0, 10)}</span>
                </div>
              )}
              {v.recusadoPor && (
                <div>
                  Recusada por <strong className="text-signal-stop">{v.recusadoPor}</strong>
                </div>
              )}

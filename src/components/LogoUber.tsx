/**
 * Marca Uber, do arquivo em `public/marca/uber-svgrepo-com.svg`.
 *
 * Inline em vez de `<img>` para o traço herdar `currentColor` — como imagem
 * externa ela viria presa no preto do arquivo e não acompanharia o tom do
 * cabeçalho.
 *
 * É desenho de preenchimento, não de traço: a animação de entrada dela é
 * opacidade e deslocamento, e não o desenho progressivo da marca VW, que só
 * funciona sobre `stroke`.
 */
export function LogoUber({
  className = "",
  titulo = "Uber",
  decorativo = false,
}: {
  ...
  return (
    <svg
      /*
       * `viewBox` recortado na caixa real do desenho (medida com `getBBox`:
       * x=1 y=10.96 w=30 h=10.07). O arquivo vem com uma moldura quadrada de
       * 32×32 e a palavra achatada no meio, então dimensionar por altura fazia
       * a marca sair minúscula com vazio em cima e embaixo.
       */
      viewBox="1 10.96 30 10.07"
      className={className}
      fill="currentColor"
      {...(decorativo

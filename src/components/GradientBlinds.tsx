...
/*
 * GradientBlinds — de reactbits.dev/backgrounds/gradient-blinds (David Haz, MIT),
 * copiado com a implementação original e adaptado no mínimo necessário:
 *
 *  - "use client", porque no App Router isto é WebGL e precisa do navegador.
 *  - `prefers-reduced-motion` pausa a animação; o shader continua desenhando o
 *    quadro parado, então o fundo não some, só deixa de se mexer.
 *
 * Fica por cima do `bg-profundo` da dobra, não no lugar dele: se o WebGL falhar
 * ou o dispositivo não tiver contexto disponível, o gradiente CSS continua lá e
 * a tela segue legível. Fundo de hero não pode depender de GPU.
 */
...
`;
    ...
    // No `window`, e não no canvas: o fundo é decoração e vive sob
    // `pointer-events: none` para não roubar o clique do conteúdo — e um
